...
[.cli-opt]#*--remove-base-image*#::
  Also remove the cached base OS image used by the cluster. The base image is
  not removed if another cluster still uses the same OS.
--
...
[.cli-opt]#*--update-os-image*#::
  Force download of OS image before deployment, even when already present in
  host local registry.
...
[.cli-opt]#*images*#::
  ...
  images to download. With [.cli-opt]#*--remove*# option, remove a cached base
  OS image instead.
+
--
This command accepts the following options:

[.cli-opt]#*--remove*=#[.cli-optval]##_OS_##::
  Remove a cached base OS image. The base image is not removed if a cluster
  still uses this OS. Run `firehpc images` without option to get the list of
  supported OS.
--
